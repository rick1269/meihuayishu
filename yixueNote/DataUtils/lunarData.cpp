...
lunar lunarData::getLunar(int y, int m, int d)
{
...
std::string lunarData::getLunarString(int year, int month, int day, int hour, int minute){
	// 1901年2月4日立春，2月19，正月初一
	lunar lunarRes = getLunar(year, month, day);
	
	std::string sYear = std::to_string(lunarRes.year);
	std::string sMonth = std::to_string(lunarRes.month);
	std::string sDay = std::to_string(lunarRes.day);
	std::string result = sYear + "年 " + sMonth + "月" + sDay + "日";
	return result;
}
